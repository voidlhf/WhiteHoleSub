(function () {
    ...
    try {
        ...
        document.title = title.replace("《", "").replace("》", "").replace("在线观看 - 达达龟", "");
    } catch (err) { }
    ...
    try {
        if (!document.querySelector("#newSearchButton1")) {
            ...
            searchButton.setAttribute("href", "https://www.dadagui.me/");
        ...
        try {
            ...
            for (var i = 0; i < elements.length; i++) {
                ...
                for (var j = 0; j < includeText.length; j++) {
                    var tempStr2 = includeText[j];
                    if (tempStr1.indexOf(tempStr2) != -1) {
                        elements[i].style.cssText = "color:#FF9900 !important";
                    }
                }
            }
        } catch (err) { }
